<x-app-layout>
        ...
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white :bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-4">
                <div class="p-3 relative border overflow-x-auto shadow-md sm:rounded-lg">
                    <a href="{{ route('stock-barang.createminuman') }}" class="bg-blue-500 text-white px-4 py-2 rounded-lg mb-4 inline-block">Add New Product</a>
                    @if($drinks->isEmpty())
                        <p>No drink found.</p>
                    @else
                                    ...
                                    @foreach($drinks as $product)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 text-center text-sm text-gray-500 flex justify-center space-x-2">
                                                ...
                                                <form action="{{ route('stock-barang.deleteminuman', $product->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this product?');">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <script>
        function openModal(productId, productName) {
            ...
            document.getElementById('quantityForm').action = '/stock-barang/update-quantity-minuman/' + productId;
        ...
        function openPriceModal(productId, productName, productPrice) {
            ...
            document.getElementById('priceForm').action = '/stock-barang/update-price-minuman/' + productId;
